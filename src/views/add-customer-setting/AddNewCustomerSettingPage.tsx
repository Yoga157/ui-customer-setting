...
import {
  selectCustomerSettingByCustomerId,
  selectPostResponseCustomerSetting,
...
const AddNewCustomerSettingPage: React.FC<IProps> = (
    ...
    dispatch(
      ModalFirstLevelActions.OPEN(
        <ModalNewInvoicingCondition
          invoicingConditionData={invoicingConditionData}
          setInvoicingConditionData={setInvoicingConditionData}
        />,
        ModalSizeEnum.Small
      )
    );
  ...
  const deleteInvoicingCondition = useCallback(
    (index: number): void => {
      dispatch(
        ModalFirstLevelActions.OPEN(
          <DeletePopUp
            array={invoicingConditionData}
            setArray={setInvoicingConditionData}
            index={index}
    ...
    dispatch(
      ModalFirstLevelActions.OPEN(
        <ModalNewRelatedCondition
          relatedCustomerData={relatedCustomerData}
          setRelatedCustomerData={setRelatedCustomerData}
        />,
        ModalSizeEnum.Tiny
      )
    );
  ...
  const deleteRelatedCustomer = useCallback(
    (index: number): void => {
      dispatch(
        ModalFirstLevelActions.OPEN(
          <DeletePopUp
            array={relatedCustomerData}
            setArray={setRelatedCustomerData}
            index={index}
  ...
  const onAddRelatedFile = useCallback((): void => {
    dispatch(
      ModalFirstLevelActions.OPEN(
        <ModalNewRelatedFile
          relatedFileData={relatedFileData}
          setRelatedFileData={setRelatedFileData}
        />,
        ModalSizeEnum.Small
      )
    );
  ...
  const deleteRelatedFile = useCallback(
    (index: number): void => {
      dispatch(
        ModalFirstLevelActions.OPEN(
          <DeletePopUp
            array={relatedFileData}
            setArray={setRelatedFileData}
            index={index}
  ...
  const postResponse = useSelector((state: IStore) =>
    selectPostResponseCustomerSetting(state)
  );
  ...
  useEffect(() => {
    if (postResponse.customerSettingID != undefined) {
      ...
      relatedFileData.map((data) => {
        ...
        formData.append(
          "createUserID",
          data.createUserID == undefined ? "0" : data.createUserID
        );

        try {
          const endpoint: string = environment.api.customer.replace(
            ":controller",
            "CustomerSetting/RelatedFile"
          );
      ...
      dispatch(
        ToastsAction.add(
          "Insert new customer setting data success!",
      ...
      <div className="form-container">
        ...
        <div className="padding-horizontal customer-search-container">
          <div>
            ...
            />
            <p className="customer-foot-note">
                  ...
                  {salesAssign.map((data) => {
                    return (
                      <div className="sales-container" key={data.salesID}>
                        <div className="sales-name">{data.salesName}</div>
                        <div
                          className="sales-delete"
                            ...
                            <Table.Row key={index}>
                              ...
                              <Table.Cell>
                                <div
                                  ...
                                  onClick={() =>
                                    deleteInvoicingCondition(index)
                                  }
                                >
                                  <Icon
                          ...
                          relatedCustomerData.map((data, index) => (
                            <Table.Row key={index}>
                              ...
                              <Table.Cell>
                                <div
                                  className="trash-container"
                                  onClick={() => deleteRelatedCustomer(index)}
                                >
                                  <Icon
                                    className="trash-icon"
                                    name="trash alternate"
